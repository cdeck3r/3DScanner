...
disable_homie_camnode() {
    ...
    disable_cmd="hostname | grep -qi centralnode || export XDG_RUNTIME_DIR=/run/user/$(${remote_user_id}) && systemctl --user is-active --quiet homie_camnode.service; if [ \$? -eq 0 ]; then systemctl --user stop homie_camnode.service && systemctl --user disable homie_camnode.service; else echo 'WARN: homie_camnode.service not active. No action required.'; fi"
