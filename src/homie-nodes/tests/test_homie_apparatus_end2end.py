...
class TestHomieApparatusEnd2End:
    ...
    def test_homie_apparatus_storage_dir_count(self, host, pytestconfig, img_dir):
        ...
        ValueStorage.dir_count = int(host.run('ls -l ' + img_dir + ' | wc -l').stdout.rstrip())
        assert ValueStorage.dir_count >= 1 # if there is no dir, there is a line in the ls -l output

    @pytest.mark.parametrize('waiting_time', [2])
    def test_homie_apparatus_camera_shutter_button(self, host, pytestconfig, waiting_time):
    ...
    @pytest.mark.parametrize('waiting_time', [2])
    def test_homie_apparatus_cameras_last_button_push_check(self, host, pytestconfig, waiting_time):
        time.sleep(waiting_time)
        msg = self.mqtt_sub(
            pytestconfig, 'scanner/apparatus/cameras/last-button-push'
        )
    ...
    @pytest.mark.parametrize('waiting_time', [2])
    def test_homie_apparatus_recent_images_save_all(self, host, pytestconfig, waiting_time):
    ...
    def test_homie_apparatus_storage_dir_count_check(self, host, pytestconfig, img_dir):
        ...
        assert int(host.run('ls -l ' + img_dir + ' | wc -l').stdout.rstrip()) > ValueStorage.dir_count 
